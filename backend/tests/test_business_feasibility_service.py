...
def test_business_feasibility_endpoint_business_7(client):
    ...
    # Market benchmark (88.0) is separated from final score
    assert data["market_benchmark_score"] == 88.0
    assert 50.0 <= data["final_score"] <= 55.0  # Approx 51.65

    # Check 5 criteria traces are present
    ...
    # Market: 88/100 -> contribution = 26.4
    assert crit_map["market"]["raw_score"] == 88.0
    assert crit_map["market"]["maximum_points"] == 30
    assert crit_map["market"]["contribution"] == 26.4

    # Financial: 10.0/100 (100,000 / 1,000,000 * 100) -> contribution = 2.5
    assert crit_map["financial"]["raw_score"] == 10.0
    assert crit_map["financial"]["maximum_points"] == 25
    assert crit_map["financial"]["contribution"] == 2.5
