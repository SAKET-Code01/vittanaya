...
class FundingStructureResponse(BaseModel):
    ...
    own_margin_capital: float = Field(..., description="Promoter margin capital committed in INR")
    required_margin_capital: Optional[float] = Field(None, description="Required promoter margin by statutory financing rule in INR")
    margin_shortfall: Optional[float] = Field(None, description="Promoter margin shortfall against statutory requirement in INR")
    margin_pct: float = Field(..., description="Margin percentage applied")
    loan_amount: float = Field(..., description="Net bank loan amount (project_cost - required margin)")
